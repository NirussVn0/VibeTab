...
│
├── ContextMenu.vue (portal)        (right-click menu, edit mode only)
│   ├── Position Actions            (center, centerH, centerV)
│   ├── Edit Settings Action        (opens Settings panel)
│   └── Delete Action               (removes widget)
│
├── SettingsPanel.vue (portal)      (settings modal, conditional)
│   ├── GeneralTab                  (expandable Clock/Weather/Search sections)
│   │   ├── ClockSettings           (show/hide, add widget)
│   │   ├── WeatherSettings         (show/hide, location, add widget)
│   │   └── SearchSettings          (add widget)
│   ├── AppearanceTab.vue           (theme, colors)
│   ├── BackgroundsTab.vue          (background management, blur slider)
│   └── AdvancedTab.vue             (advanced options)
│
├── CommandPalette.vue (portal)     (cmd palette, conditional)
│   ├── SearchInput                 (search input with AI prefix)
│   ├── ActionResults               (filtered commands)
│   ├── AISearchPrompt              (when query starts with >)
│   └── GoogleSearchPrompt          (fallback search)
│
